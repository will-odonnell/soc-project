...
CMatlibVector<CReal>	Rand(const int iLen)
							{_VECOP(CReal, iLen, (CReal) 0xDEADBEEF );}
...
inline void		IIR1(IComplex& cY, const IComplex& cX, const _INT rLambda)
							{cY = rLambda * (cY - cX) + cX;}

inline void		IIR1(int& rY, const int& rX, const int rLambda)
							{rY = rLambda * (rY - rX) + rX;}
