...
typedef int 			CFix;
typedef complex<int>		FComplex;
typedef CMatlibVector<CReal>	CRealVector;
typedef CMatlibVector<int>	intVector;
...
typedef CMatlibMatrix<int>	intMatrix;
...
//		pData = new CComplex[iVectorLength];
	...
	CMatlibVector(const CMatlibVector<int>& fvReal, const CMatlibVector<int>& fvImag) : 
		eVType(VTY_CONST/*VTY_TEMP*/), iVectorLength(fvReal.GetSize()), pData(NULL)
	{
		/* Allocate data block for vector */
...
inline CMatlibVector<int> // rv, rv
	operator*(const CMatlibVector<int>& rvA, const CMatlibVector<int>& rvB)
	{_VECOP(int, rvA.GetSize(), rvA[i] * rvB[i]);}
...
inline CMatlibVector<FComplex> // cv, rv
	operator*(const CMatlibVector<FComplex>& cvA, const CMatlibVector<int>& rvB)
...
inline CMatlibVector<FComplex> // rv, cv 
        operator*(const CMatlibVector<int>& rvB, const CMatlibVector<FComplex>& cvA) 
        {_VECOP(FComplex, cvA.GetSize(), cvA[i] * rvB[i]);} 
...
template<class T> inline
CMatlibVector<T> // Tv, r 
        operator*(const CMatlibVector<T>& vecA, const int& rB) 
        {_VECOP(T, vecA.GetSize(), vecA[i] * rB);} 
...
template<class T> inline 
CMatlibVector<T> // r, Tv 
        operator*(const int& rA, const CMatlibVector<T>& vecB) 
        {_VECOP(T, vecB.GetSize(), rA * vecB[i]);} 
...
inline CMatlibVector<FComplex> // rv, cv 
        operator/(const CMatlibVector<int>& rvA, const CMatlibVector<FComplex>& cvB) 
...
CMatlibVector<T> CMatlibVector<T>::operator()(const int iFrom,
						  ...
						  const int iTo) const
{
	CMatlibVector<T> vecRet((iTo - iFrom) / (iStep) + 1, VTY_TEMP);
	int iOutPos = 0;
	int i;
...
public:
	...
	CMatlibMatrix(const int iNRowLen, const int iNColLen,
		...
		eVType(eNTy), iRowSize(0), ppData(NULL) {Init(iNRowLen, iNColLen, T(0));}
	...
	inline CMatlibMatrix<T>& operator=(const CMatlibMatrix<int>& matI)  
	  {_TESTSIZEM(matI.GetRowSize()); _MATOPCL(= matI[i]);} 
	...
	inline CMatlibMatrix<T>& operator*=(const CReal& rI)
		{_MATOPCL(*= rI);}
	inline CMatlibMatrix<T>& operator*=(const int& rI) 
        	{_MATOPCL(*= rI);} 
