			     ...
			     FXP& rLenPDS_fxp, FXP& rOffsPDS_fxp)
{
  ...
  // sglee 
  int				n(veccPilots.GetSize());
  // printf ("vec size : %d \n", n);
  
  veccPilots_fxp.Init(n);

  for (int i = 0; i < n; i++)
  {
    veccPilots_fxp[i] = (FComplex(FXP(veccPilots[i].real()), FXP(veccPilots[i].imag())));
  }

  veccPilots_fxp = Ifft(veccPilots_fxp, FftPlan);
  
  for (int i = 0; i < n; i++)
  {
    veccPilots[i] = CComplex((double)veccPilots_fxp[i].real(), (double)veccPilots_fxp[i].imag());
  }
