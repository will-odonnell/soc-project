						    ...
						    CVector<_COMPLEX>& veccPilotCells, _REAL rSNR)
{
	...
	for (j = 0; j < iLenHistBuff - 1; j++)
	{
		for (i = 0; i < iNumIntpFreqPil; i++) 
                { 
                        matcChanEstHist_FX[j][i] = matcChanEstHist_FX[j + 1][i];
                }
	}

	/* Clear current symbol for new channel estimates */
	for (i = 0; i < iNumIntpFreqPil; i++) {
                matcChanEstHist_FX[iLenHistBuff - 1][i] = 
			_FCOMPLEX((FXP) 0, (FXP) 0);
	}
		...
		if (_IsScatPil(veciMapTab[i]))
		{
			...
			/* h = r / s, h: transfer function of channel, r: received signal, 
			   s: transmitted signal */
                        ...
                        matcChanEstHist_FX[iLenHistBuff - 1][iPiHiIndex] = 
			...
			{
			        CComplex matcChanEHisOld;
                                matcChanEHisOld  = matcChanEstHist_FX[0][iPiHiIndex];
			        CComplex matcChanEHisNew;
				matcChanEHisNew = matcChanEstHist_FX[iLenHistBuff - 1][iPiHiIndex];
				...
				cOldPilot = Rotate(matcChanEHisOld, i, iTimeDiffOld);
				cNewPilot = Rotate(matcChanEHisNew, i, iTimeDiffNew);


				/* Linear interpolation ------------------------------------- */
				/* First, calculate gradient */
                                cGrad_FX = (cNewPilot - cOldPilot) / (_REAL)(iLenHistBuff - 1);

				/* Apply linear interpolation to cells in between */
                                matcChanEstHist_FX[j][iPiHiIndex] = cGrad_FX * (FXP) j;
				matcChanEstHist_FX[j][iPiHiIndex] += cOldPilot;
	...
	for (i = 0; i < iNumIntpFreqPil; i++)
		veccOutputData[i] = matcChanEstHist_FX[0][i];
...
int CTimeLinear::Init(CParameter& Parameter)
{
	...
	matcChanEstHist_FX.Init(iLenHistBuff, iNumIntpFreqPil, 
		_FCOMPLEX((FXP) 1, (FXP) 0));
