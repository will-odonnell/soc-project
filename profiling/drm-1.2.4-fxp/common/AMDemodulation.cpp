...
void CAMDemodulation::ProcessDataInternal(CParameter& ReceiverParam)
{
	...
	for (i = 0; i < iInputBlockSize; i++)
	  rvecInpTmp[i] = (_REAL) (*pvecInputData)[i];
...
\******************************************************************************/
_BOOLEAN CFreqOffsAcq::Run(const CVector<_FREAL>& vecrInpData)
{
	/* Init return flag */
	...
	if (bAcquisition == TRUE)
	{
		...
		else
		{
			...
			for (int i = 0; i < iTotalBufferSize; i++)
			  vecrFFTInput[i] = (_REAL) vecrFFTHistory[i];
...
void CFreqOffsAcq::Init(const int iNewBlockSize)
{
	...
	vecrFFTHistory.Init(iTotalBufferSize, (CFReal) 0.0);
