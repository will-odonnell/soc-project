...
void COFDMDemodulation::ProcessDataInternal(CParameter& ReceiverParam)
{
	...
	// sglee
        int				n(veccFFTInput.GetSize());
        //printf ("vec size : %d \n", n);
        
	int SCALE = 8;

        veccFFTInput_fxp.Init(n);
        veccFFTOutput_fxp.Init(n);
   
        for (int i = 0; i < n; i++)
        {
          veccFFTInput_fxp[i] = FComplex(FXP(veccFFTInput[i].real()/SCALE), FXP(veccFFTInput[i].imag()/SCALE));
        }

	// sglee veccFFTOutput = Fft(veccFFTInput, FftPlan);
	veccFFTOutput_fxp = Fft(veccFFTInput_fxp, FftPlan);

        for (int i = 0; i < n; i++)
        {
          veccFFTOutput[i] = CComplex((double)veccFFTOutput_fxp[i].real()*SCALE, (double)veccFFTOutput_fxp[i].imag()*SCALE);
        }

        //printf ("Fft fxp output : %f\n", veccFFTOutput[100].real());

	// original FFT
      //  printf ("Fft flp input : %f\n", veccFFTInput[100].real());

	//veccFFTOutput = Fft(veccFFTInput, FftPlan);

        //for (int i = 0; i < n; i++)
        //{
        //  veccFFTOutput[i] = CComplex(veccFFTOutput[i].real()*256, veccFFTOutput[i].imag()*256);
        //} 
        //printf ("Fft flp output : %f\n", veccFFTOutput[100].real());
