							...
							const CFftPlans& FftPlans)
{
	...
	_COMPLEX* pFftwComplexIn=new _COMPLEX[n];
	_COMPLEX* pFftwComplexOut=new _COMPLEX[n];
	...
	for (i = 0; i < n; i++)
		cvReturn[i] = CComplex(pFftwComplexOut[i].real(), pFftwComplexOut[i].imag());
							 ...
							 const CFftPlans& FftPlans)
{
	...
	for (i = 0; i < n; i++)
	{
		cvReturn[i] = CComplex(pFftwComplexOut[i].real() * scale,
		...
		const CFftPlans& FftPlans)
{
	...
	_REAL *pFftwRealIn = new _REAL[iSizeI];
	_COMPLEX* pFftwComplexOut = new _COMPLEX[iShortLength+1];
		...
		const CFftPlans& FftPlans)
{
	...
	_COMPLEX* pFftwComplexIn = new _COMPLEX[cvI.GetSize()];
	_REAL* pFftwRealOut=new _REAL[iLongLength];

	for(i=0; i<cvI.GetSize(); i++)
	{
		pFftwComplexIn[i]=_COMPLEX(cvI.pData[i].real(), cvI[i].imag());
	}
