...
int32_t mp4ff_write_int32(mp4ff_t *f,const uint32_t data)
{
    ...
    *(uint32_t*)temp = data;
    ...
    return mp4ff_write_data(f,(int8_t*)((void *)&result),sizeof(result));
}
...
uint64_t mp4ff_read_int64(mp4ff_t *f)
{
    ...
    mp4ff_read_data(f, (int8_t *)((void *)data), 8);
...
char * mp4ff_read_string(mp4ff_t * f,uint32_t length)
{
	...
	if (str!=0)
	{
		if ((uint32_t)mp4ff_read_data(f,(int8_t *)((void *)str),length)!=length)
		{
			free(str);
			str = 0;
		}
...
{
    uint8_t output;
    mp4ff_read_data(f, (int8_t *)((void *)&output), 1);
    return output;
}
